...
def get_avaliacoes(db: Session, motorista_id: int, skip: int = 0, limit: int = 100) -> list[AvaliacaoResponse]:
    return db.query(Avaliacao).filter(Avaliacao.motorista_id == motorista_id).offset(skip).limit(limit).all()

def get_nota_media(db: Session, motorista_id: int) -> float:
    avaliacoes = db.query(Avaliacao).filter(Avaliacao.motorista_id == motorista_id).all()
    if not avaliacoes:
        return 0.0
